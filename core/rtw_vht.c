...
void	update_sta_vht_info_apmode(_adapter *padapter, PVOID sta)
{
	...
	rtl8188fu__rtw_memcpy(pvhtpriv_sta->vht_mcs_map, pcap_mcs, 2);
...
void	update_hw_vht_param(_adapter *padapter)
{
	...
	if(pvhtpriv->ampdu_len > ht_AMPDU_len)
		rtl8188fu_rtw_hal_set_hwreg(padapter, HW_VAR_AMPDU_FACTOR, (u8 *)(&pvhtpriv->ampdu_len));
}
...
void Vrtl8188fu_HT_caps_handler(_adapter *padapter, PNDIS_802_11_VARIABLE_IEs pIE)
{
	...
	rtl8188fu__rtw_memcpy(pvhtpriv->vht_mcs_map, vht_mcs, 2);
...
void rtw_process_vht_op_mode_notify(_adapter *padapter, u8 *pframe, PVOID sta)
{
	...
	if (target_bw != psta->bw_mode) {
		if (rtl8188fu_hal_is_bw_support(padapter, target_bw)
	...
	if (target_rxss != current_rxss) {
		...
		rtl8188fu__rtw_memcpy(psta->vhtpriv.vht_mcs_map, vht_mcs_map, 2);
...
u32	rtw_build_vht_operation_ie(_adapter *padapter, u8 *pbuf, u8 channel)
{
	...
	rtl8188fu__rtw_memset(operation, 0, 5);

	bw_mode = REGSTY_BW_5G(pregistrypriv); /* TODO: control op bw with other info */

	if (rtl8188fu_hal_chk_bw_cap(padapter, BW_CAP_80M | BW_CAP_160M)
		&& REGSTY_BW_5G(pregistrypriv) >= CHANNEL_WIDTH_80
	) {
		center_freq = rtl8188fu_rtw_get_center_ch(channel, bw_mode, HAL_PRIME_CHNL_OFFSET_LOWER);
		ChnlWidth = 1;
	} else {
		center_freq = 0;
		ChnlWidth = 0;
	...
	rtl8188fu_rtw_set_ie(pbuf, EID_VHTOperation, 5, operation, &len);
...
u32	rtw_build_vht_op_mode_notify_ie(_adapter *padapter, u8 *pbuf, u8 bw)
{
	...
	pbuf = rtl8188fu_rtw_set_ie(pbuf, EID_OpModeNotification, 1, &opmode, &len);
...
u32	rtw_build_vht_cap_ie(_adapter *padapter, u8 *pbuf)
{
	...
	rtl8188fu__rtw_memset(pcap, 0, 32);
	...
	if (rtl8188fu_hal_chk_bw_cap(padapter, BW_CAP_160M) && REGSTY_IS_BW_5G_SUPPORT(pregistrypriv, CHANNEL_WIDTH_160)) {
		if (rtl8188fu_hal_chk_bw_cap(padapter, BW_CAP_80_80M) && REGSTY_IS_BW_5G_SUPPORT(pregistrypriv, CHANNEL_WIDTH_80_80))
	...
	rtl8188fu__rtw_memcpy(pcap_mcs, pvhtpriv->vht_mcs_map, 2);

	pcap_mcs = GET_VHT_CAPABILITY_ELE_TX_MCS(pcap);
	rtl8188fu__rtw_memcpy(pcap_mcs, pvhtpriv->vht_mcs_map, 2);

	/* find the largest bw supported by both registry and hal */
	bw = rtl8188fu_hal_largest_bw(padapter, REGSTY_BW_5G(pregistrypriv));
	...
	pbuf = rtl8188fu_rtw_set_ie(pbuf, EID_VHTCapability, 12, pcap, &len);
...
u32 rtw_restructure_vht_ie(_adapter *padapter, u8 *in_ie, u8 *out_ie, uint in_len, uint *pout_len)
{
	...
	p = rtl8188fu_rtw_get_ie(in_ie+12, EID_VHTCapability, &ielen, in_len-12);
	if (p && ielen>0) {
		...
		*pout_len += cap_len;
		...
		// Get HT BW
		p = rtl8188fu_rtw_get_ie(in_ie+12, _HT_EXTRA_INFO_IE_, &ielen, in_len-12);
		if (p && ielen>0) {
		...
		p = rtl8188fu_rtw_get_ie(in_ie+12, EID_VHTOperation, &ielen, in_len-12);
		if (p && ielen>0) {
			out_len = *pout_len;
			...
			pframe = rtl8188fu_rtw_set_ie(out_ie+out_len, EID_VHTOperation, ielen, p+2 , pout_len);
		}

		/* find the largest bw supported by both registry and hal */
		notify_bw = rtl8188fu_hal_largest_bw(padapter, REGSTY_BW_5G(pregistrypriv));
...
void Vrtl8188fu_HTrtl8188fu_OnAssocRsp(_adapter *padapter)
{
	...
	rtl8188fu_rtw_hal_set_hwreg(padapter, HW_VAR_AMPDU_MAX_TIME, (u8 *)(&pvhtpriv->vht_highest_rate));
}
