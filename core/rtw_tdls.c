...
extern void rtl8188fu_process_wmmps_data(_adapter *padapter, union recv_frame *precv_frame);
...
void rtw_reset_tdls_info(_adapter* padapter)
{
	...
	rtl8188fu_ATOMIC_SET(&ptdlsinfo->chsw_info.chsw_on, _FALSE);
...
int rtw_init_tdls_info(_adapter* padapter)
{
	...
	rtl8188fu__rtw_spinlock_init(&ptdlsinfo->cmd_lock);
	rtl8188fu__rtw_spinlock_init(&ptdlsinfo->hdl_lock);
...
{
	rtl8188fu__rtw_spinlock_free(&ptdlsinfo->cmd_lock);
	rtl8188fu__rtw_spinlock_free(&ptdlsinfo->hdl_lock);

	rtl8188fu__rtw_memset(ptdlsinfo, 0, sizeof(struct tdls_info) );
...
int _rtl8188fu_issue_nulldata_to_TDLS_peer_STA(_adapter *padapter, unsigned char *da, unsigned int power_mode, int wait_ack)
{
	int ret = _FAIL;
 ...
 */
int rtl8188fu_issue_nulldata_to_TDLS_peer_STA(_adapter *padapter, unsigned char *da, unsigned int power_mode, int try_cnt, int wait_ms)
{
	int ret;
	int i = 0;
	...
	do {
		ret = _rtl8188fu_issue_nulldata_to_TDLS_peer_STA(padapter, da, power_mode, wait_ms>0 ? _TRUE : _FALSE);
...
void free_tdls_sta(_adapter *padapter, struct sta_info *ptdls_sta)
{
	...
	if (ptdlsinfo->sta_cnt < MAX_ALLOWED_TDLS_STA_NUM) {
		ptdlsinfo->sta_maximum = _FALSE;
		rtl8188fu__rtw_memset( &ptdlsinfo->ss_record, 0x00, sizeof(struct tdls_ss_record) );
	}

	/* clear cam */
	rtl8188fu_rtw_clearstakey_cmd(padapter, ptdls_sta, _TRUE);
	...
	rtl8188fu_rtw_free_stainfo(padapter,  ptdls_sta);
...
void rtw_tdls_set_key(_adapter *padapter, struct sta_info *ptdls_sta)
{
	...
	rtl8188fu_rtw_setstakey_cmd(padapter, ptdls_sta, TDLS_KEY, _TRUE);
}
...
void rtw_tdls_process_ht_cap(_adapter *padapter, struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	rtl8188fu__rtw_memset(&ptdls_sta->htpriv.ht_cap, 0, sizeof(struct rtw_ieee80211_ht_cap));
	if (data && Length >= sizeof(struct rtw_ieee80211_ht_cap)) {
		ptdls_sta->flags |= WLAN_STA_HT;
		ptdls_sta->flags |= WLAN_STA_WME;

		rtl8188fu__rtw_memcpy(&ptdls_sta->htpriv.ht_cap, data, sizeof(struct rtw_ieee80211_ht_cap));			
	} else
		ptdls_sta->flags &= ~WLAN_STA_HT;
...
u8 *rtw_tdls_set_ht_cap(_adapter *padapter, u8 *pframe, struct pkt_attrib *pattrib)
{
	...
	rtl8188fu_rtw_restructure_ht_ie(padapter, NULL, pframe, 0, &(pattrib->pktlen), padapter->mlmeextpriv.cur_channel);
...
void rtw_tdls_process_vht_cap(_adapter *padapter, struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	rtl8188fu__rtw_memset(&ptdls_sta->vhtpriv, 0, sizeof(struct vht_priv));
	if (data && Length == 12) {
		ptdls_sta->flags |= WLAN_STA_VHT;

		rtl8188fu__rtw_memcpy(ptdls_sta->vhtpriv.vht_cap, data, 12);
	...
	rtl8188fu__rtw_memcpy(vht_mcs, pcap_mcs, 2);
	...
	rtl8188fu__rtw_memcpy(ptdls_sta->vhtpriv.vht_mcs_map, vht_mcs, 2);
...
{
	return rtl8188fu_rtw_set_ie(pframe, EID_AID, 2, (u8 *)&(padapter->mlmepriv.cur_network.aid), &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_sup_ch(struct mlme_ext_priv *pmlmeext, u8 *pframe, struct pkt_attrib *pattrib)
{
	...
	return rtl8188fu_rtw_set_ie(pframe, _SUPPORTED_CH_IE_, sup_ch_idx, sup_ch, &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_rsnie(struct tdls_txmgmt *ptxmgmt, u8 *pframe, struct pkt_attrib *pattrib,  int init, struct sta_info *ptdls_sta)
{
	...
	if (ptxmgmt->len > 0)
		p = rtl8188fu_rtw_get_ie(ptxmgmt->buf, _RSN_IE_2_, &len, ptxmgmt->len);

	if (p != NULL)
		return rtl8188fu_rtw_set_ie(pframe, _RSN_IE_2_, len, p+2, &(pattrib->pktlen));
	else
		if (init == _TRUE)
			return rtl8188fu_rtw_set_ie(pframe, _RSN_IE_2_, sizeof(TDLS_RSNIE), TDLS_RSNIE, &(pattrib->pktlen));
		else
			return rtl8188fu_rtw_set_ie(pframe, _RSN_IE_2_, sizeof(ptdls_sta->TDLS_RSNIE), ptdls_sta->TDLS_RSNIE, &(pattrib->pktlen));
}

u8 *rtw_tdls_set_ext_cap(u8 *pframe, struct pkt_attrib *pattrib)
{
	return rtl8188fu_rtw_set_ie(pframe, _EXT_CAP_IE_ , sizeof(TDLS_EXT_CAPIE), TDLS_EXT_CAPIE, &(pattrib->pktlen));
}

u8 *rtw_tdls_set_qos_cap(u8 *pframe, struct pkt_attrib *pattrib)
{
	return rtl8188fu_rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_, sizeof(TDLS_WMMIE), TDLS_WMMIE,  &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_ftie(struct tdls_txmgmt *ptxmgmt, u8 *pframe, struct pkt_attrib *pattrib, u8 *ANonce, u8 *SNonce)
{
	...
	if (ptxmgmt->len > 0)
		p = rtl8188fu_rtw_get_ie(ptxmgmt->buf, _FTIE_, &len, ptxmgmt->len);

	if (p != NULL)
		return rtl8188fu_rtw_set_ie(pframe, _FTIE_, len, p+2, &(pattrib->pktlen));
	else {
		if (ANonce != NULL)
			rtl8188fu__rtw_memcpy(FTIE.Anonce, ANonce, WPA_NONCE_LEN);
		if (SNonce != NULL)
			rtl8188fu__rtw_memcpy(FTIE.Snonce, SNonce, WPA_NONCE_LEN);
		return rtl8188fu_rtw_set_ie(pframe, _FTIE_ , 82, (u8 *)FTIE.mic_ctrl, &(pattrib->pktlen));
	}
}
...
u8 *rtw_tdls_set_timeout_interval(struct tdls_txmgmt *ptxmgmt, u8 *pframe, struct pkt_attrib *pattrib, int init, struct sta_info *ptdls_sta)
{
	...
	if (ptxmgmt->len > 0)
		p = rtl8188fu_rtw_get_ie(ptxmgmt->buf, _TIMEOUT_ITVL_IE_, &len, ptxmgmt->len);

	if (p != NULL)
		return rtl8188fu_rtw_set_ie(pframe, _TIMEOUT_ITVL_IE_, len, p+2, &(pattrib->pktlen));
	else {
		/* Timeout interval */
		timeout_itvl[0]=0x02;
		if (init == _TRUE)
			...
			rtl8188fu__rtw_memcpy(timeout_itvl+1, (u8 *)(&ptdls_sta->TDLS_PeerKey_Lifetime), 4);

		return rtl8188fu_rtw_set_ie(pframe, _TIMEOUT_ITVL_IE_, 5, timeout_itvl, &(pattrib->pktlen));
	}
}
...
u8 *rtw_tdls_set_bss_coexist(_adapter *padapter, u8 *pframe, struct pkt_attrib *pattrib)
{
	...
	iedata |= BIT(0);
	return rtl8188fu_rtw_set_ie(pframe, EID_BSSCoexistence, 1, &iedata, &(pattrib->pktlen));
}

u8 *rtw_tdls_set_payload_type(u8 *pframe, struct pkt_attrib *pattrib)
{
	u8 payload_type = 0x02;
	return rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(payload_type), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_category(u8 *pframe, struct pkt_attrib *pattrib, u8 category)
{
	return rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(category), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_action(u8 *pframe, struct pkt_attrib *pattrib, struct tdls_txmgmt *ptxmgmt)
{
	return rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(ptxmgmt->action_code), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_status_code(u8 *pframe, struct pkt_attrib *pattrib, struct tdls_txmgmt *ptxmgmt)
{
	return rtl8188fu_rtw_set_fixed_ie(pframe, 2, (u8 *)&(ptxmgmt->status_code), &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_dialog(u8 *pframe, struct pkt_attrib *pattrib, struct tdls_txmgmt *ptxmgmt)
{
	...
	if (ptxmgmt->dialog_token)
		return rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(ptxmgmt->dialog_token), &(pattrib->pktlen));
	else
		return rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(dialogtoken), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_reg_class(u8 *pframe, struct pkt_attrib *pattrib, struct sta_info *ptdls_sta)
{
	u8 reg_class = 22;
	return rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(reg_class), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_second_channel_offset(u8 *pframe, struct pkt_attrib *pattrib, u8 ch_offset)
{
	return rtl8188fu_rtw_set_ie(pframe, EID_SecondaryChnlOffset , 1, &ch_offset, &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_capability(_adapter *padapter, u8 *pframe, struct pkt_attrib *pattrib)
{
	...
	rtl8188fu__rtw_memcpy(cap_from_ie, rtl8188fu_rtw_get_capability_from_ie(pmlmeinfo->network.IEs), 2);

	return rtl8188fu_rtw_set_fixed_ie(pframe, 2, cap_from_ie, &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_supported_rate(_adapter *padapter, u8 *pframe, struct pkt_attrib *pattrib)
{
	...
	rtl8188fu_rtw_set_supported_rate(bssrate, (padapter->registrypriv.wireless_mode == WIRELESS_MODE_MAX) ? padapter->mlmeextpriv.cur_wireless_mode : padapter->registrypriv.wireless_mode); 
	bssrate_len = rtl8188fu_rtw_get_rateset_len(bssrate);
...
{
	return rtl8188fu_rtw_set_ie(pframe, _SRC_IE_ , sizeof(TDLS_SRC), TDLS_SRC, &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_linkid(u8 *pframe, struct pkt_attrib *pattrib, u8 init)
{
	...
	if (init == _TRUE) {
		rtl8188fu__rtw_memcpy(link_id_addr, pattrib->ra, 6);
		rtl8188fu__rtw_memcpy((link_id_addr+6), pattrib->src, 6);
		rtl8188fu__rtw_memcpy((link_id_addr+12), pattrib->dst, 6);
	} else {
		rtl8188fu__rtw_memcpy(link_id_addr, pattrib->ra, 6);
		rtl8188fu__rtw_memcpy((link_id_addr+6), pattrib->dst, 6);
		rtl8188fu__rtw_memcpy((link_id_addr+12), pattrib->src, 6);
	}
	return rtl8188fu_rtw_set_ie(pframe, _LINK_ID_IE_, 18, link_id_addr, &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_target_ch(_adapter *padapter, u8 *pframe, struct pkt_attrib *pattrib)
{
	...
	if (padapter->tdlsinfo.chsw_info.off_ch_num)
		return rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(padapter->tdlsinfo.chsw_info.off_ch_num), &(pattrib->pktlen));
	else
		return rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(target_ch), &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_ch_sw(u8 *pframe, struct pkt_attrib *pattrib, struct sta_info *ptdls_sta)
{
	...
	rtl8188fu__rtw_memcpy(ch_switch_timing, &switch_time, 2);
	rtl8188fu__rtw_memcpy(ch_switch_timing + 2, &switch_timeout, 2);

	return rtl8188fu_rtw_set_ie(pframe, _CH_SWITCH_TIMING_,  4, ch_switch_timing, &(pattrib->pktlen));
}

void rtw_tdls_set_ch_sw_oper_control(_adapter *padapter, u8 enable)
{
	if (rtl8188fu_ATOMIC_READ(&padapter->tdlsinfo.chsw_info.chsw_on) != enable)
		rtl8188fu_ATOMIC_SET(&padapter->tdlsinfo.chsw_info.chsw_on, enable);

	rtl8188fu_rtw_hal_set_hwreg(padapter, HW_VAR_TDLS_BCN_EARLY_C2H_RPT, &enable);
...
void rtw_tdls_ch_sw_back_to_base_chnl(_adapter *padapter)
{
	...
	(padapter->mlmeextpriv.cur_channel != rtl8188fu_rtw_get_oper_ch(padapter))) {
		//if(pchsw_info->ch_sw_state & TDLS_CH_SW_INITIATOR_STATE)				
		rtl8188fu_rtw_tdls_cmd(padapter, pchsw_info->addr, TDLS_CH_SW_TO_BASE_CHNL_UNSOLICITED);
	}			
}
...
static void rtw_tdls_chsw_oper_init(_adapter* padapter, u32 timeout_ms)
{
	...
	rtl8188fu_rtw_sctx_init(chsw_sctx, timeout_ms);
}
...
static int rtw_tdls_chsw_oper_wait(_adapter* padapter)
{
	...
	return rtl8188fu_rtw_sctx_wait(chsw_sctx, __func__);
}
...
void rtw_tdls_chsw_oper_done(_adapter* padapter)
{
	...
	rtl8188fu_rtw_sctx_done(&chsw_sctx);
}
...
s32 rtw_tdls_do_ch_sw(_adapter *padapter, struct sta_info *ptdls_sta, u8 chnl_type, u8 channel, u8 channel_offset, u16 bwmode, u16 ch_switch_time)
{
	...
	ch_sw_time_start = rtl8188fu_rtw_systime_to_ms(rtl8188fu_rtw_get_current_time());

	rtw_tdls_chsw_oper_init(padapter, TDLS_CH_SWITCH_OPER_OFFLOAD_TIMEOUT);

	/* set mac_id sleep before channel switch */
	rtl8188fu_rtw_hal_macid_sleep(padapter, ptdls_sta->mac_id);
	...
	if (rtw_hal_ch_sw_oper_offload(padapter, channel, channel_offset, bwmode) == _SUCCESS) {
		if (rtw_tdls_chsw_oper_wait(padapter) == _SUCCESS) {
			...
			if (take_care_iqk == _TRUE)
				rtl8188fu_rtw_hal_ch_sw_iqk_info_restore(padapter, CH_SW_USE_CASE_TDLS);

			ch_sw_time_spent = rtl8188fu_rtw_systime_to_ms(rtl8188fu_rtw_get_current_time()) - ch_sw_time_start;
			...
			if (chnl_type == TDLS_CH_SW_OFF_CHNL) {
				...
				if (wait_time > 0)
					rtl8188fu_rtw_msleep_os(wait_time);
			}
	...
	rtl8188fu_rtw_hal_macid_wakeup(padapter, ptdls_sta->mac_id);
...
u8 *rtw_tdls_set_wmm_params(_adapter *padapter, u8 *pframe, struct pkt_attrib *pattrib)
{
	...
	if (&pmlmeinfo->WMM_param) {
		rtl8188fu__rtw_memcpy(wmm_param_ele, rtl8188fu_WMM_PARA_OUI, 6);
		if (rtl8188fu__rtw_memcmp(&pmlmeinfo->WMM_param, &wmm_param_ele[6], 18) == _TRUE)
			/* Use default WMM Param */
			rtl8188fu__rtw_memcpy(wmm_param_ele + 6, (u8 *)&TDLS_WMM_PARAM_IE, sizeof(TDLS_WMM_PARAM_IE));
		else	
			rtl8188fu__rtw_memcpy(wmm_param_ele + 6, (u8 *)&pmlmeinfo->WMM_param, sizeof(pmlmeinfo->WMM_param));
		return rtl8188fu_rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_,  24, wmm_param_ele, &(pattrib->pktlen));		
	}
	else
		return pframe;
}
...
void rtw_tdls_process_wfd_ie(struct tdls_info *ptdlsinfo, u8 *ptr, u8 length)
{
	...
	if (!rtl8188fu_hal_chk_wl_func(tdls_info_to_adapter(ptdlsinfo), WL_FUNC_MIRACAST))
		return;

	/* Try to get the TCP port information when receiving the negotiation response. */

	wfd_ie = rtl8188fu_rtw_get_wfd_ie(ptr, length, NULL, &wfd_ielen);
	while (wfd_ie) {
		u8 *attr_content;
		...
		attr_content = rtl8188fu_rtw_get_wfd_attr_content(wfd_ie, wfd_ielen, WFD_ATTR_LOCAL_IP_ADDR, NULL, &attr_contentlen);
		if (attr_content && attr_contentlen) {
			rtl8188fu__rtw_memcpy(ptdlsinfo->wfd_info->peer_ip_address, ( attr_content + 1 ), 4);
			DBG_871X("[%s] Peer IP = %02u.%02u.%02u.%02u\n", __FUNCTION__, 
				ptdlsinfo->wfd_info->peer_ip_address[0], ptdlsinfo->wfd_info->peer_ip_address[1],
				ptdlsinfo->wfd_info->peer_ip_address[2], ptdlsinfo->wfd_info->peer_ip_address[3]);
		}

		wfd_ie = rtl8188fu_rtw_get_wfd_ie(wfd_ie + wfd_ielen, (ptr + length) - (wfd_ie + wfd_ielen), NULL, &wfd_ielen);
	}
}
...
int issue_tunneled_probe_req(_adapter *padapter)
{
	...
	rtl8188fu__rtw_memcpy(pattrib->dst, baddr, ETH_ALEN);
	...
	if (rtl8188fu_rtw_xmit_tdls_coalesce(padapter, pmgntframe, &txmgmt) != _SUCCESS) {
...
int issue_tunneled_probe_rsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	rtl8188fu__rtw_memcpy(pattrib->dst, precv_frame->u.hdr.attrib.src, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->src, adapter_mac_addr(padapter), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ra, get_bssid(pmlmepriv), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ta, pattrib->src, ETH_ALEN);
...
int issue_tdls_setup_req(_adapter *padapter, struct tdls_txmgmt *ptxmgmt, int wait_ack)
{
	...
	rtl8188fu__rtw_memcpy(pattrib->dst, ptxmgmt->peer, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->src, adapter_mac_addr(padapter), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ra, get_bssid(pmlmepriv), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ta, pattrib->src, ETH_ALEN);
	...
	/* init peer sta_info */
	ptdls_sta = rtl8188fu_rtw_get_stainfo(pstapriv, ptxmgmt->peer);
	if (ptdls_sta == NULL) {
		ptdls_sta = rtl8188fu_rtw_alloc_stainfo(pstapriv, ptxmgmt->peer);
		if (ptdls_sta == NULL) {
			DBG_871X("[%s] rtl8188fu_rtw_alloc_stainfo fail\n", __FUNCTION__);	
			rtl8188fu_rtw_free_xmitbuf(pxmitpriv,pmgntframe->pxmitbuf);
			rtl8188fu_rtw_free_xmitframe(pxmitpriv, pmgntframe);
			goto exit;
		}
	...
	if (rtl8188fu_rtw_xmit_tdls_coalesce(padapter, pmgntframe, ptxmgmt) !=_SUCCESS) {
		rtl8188fu_rtw_free_xmitbuf(pxmitpriv,pmgntframe->pxmitbuf);
		rtl8188fu_rtw_free_xmitframe(pxmitpriv, pmgntframe);
		goto exit;
	}
...
int _issue_tdls_teardown(_adapter *padapter, struct tdls_txmgmt *ptxmgmt, u8 wait_ack)
{
	...
	rtl8188fu_rtw_set_scan_deny(padapter, 550);
	...
	if (rtw_tdls_is_driver_setup(padapter))
		rtl8188fu_rtw_tdls_cmd(padapter, ptxmgmt->peer, TDLS_TEARDOWN_STA_LOCALLY);
...
int issue_tdls_setup_rsp(_adapter *padapter, struct tdls_txmgmt *ptxmgmt)
{
	...
	rtl8188fu__rtw_memcpy(pattrib->dst, ptxmgmt->peer, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->src, adapter_mac_addr(padapter), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ra, get_bssid(&(padapter->mlmepriv)), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ta, pattrib->src, ETH_ALEN);
...
int issue_tdls_dis_rsp(_adapter *padapter, struct tdls_txmgmt *ptxmgmt, u8 privacy)
{
	...
	*(fctrl) = 0;
	...
	rtl8188fu__rtw_memcpy(pwlanhdr->addr1, ptxmgmt->peer, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->dst, pwlanhdr->addr1, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->src, pwlanhdr->addr2, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pwlanhdr->addr3, get_bssid(&padapter->mlmepriv), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ra, pwlanhdr->addr3, ETH_ALEN);
...
int issue_tdls_peer_traffic_rsp(_adapter *padapter, struct sta_info *ptdls_sta, struct tdls_txmgmt *ptxmgmt)
{
	...
	rtl8188fu__rtw_memcpy(pattrib->dst, ptdls_sta->hwaddr, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->src, adapter_mac_addr(padapter), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ra, get_bssid(pmlmepriv), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ta, pattrib->src, ETH_ALEN);
...
int issue_tdls_ch_switch_rsp(_adapter *padapter, struct tdls_txmgmt *ptxmgmt, int wait_ack)
{
	...
	rtl8188fu__rtw_memcpy(pattrib->dst, ptxmgmt->peer, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->src, adapter_mac_addr(padapter), ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ra, ptxmgmt->peer, ETH_ALEN);
	rtl8188fu__rtw_memcpy(pattrib->ta, pattrib->src, ETH_ALEN);
...
*/
	...
	if (wait_ack) {
		ret = rtl8188fu_rtl8188fu_dump_mgntframe_and_wait_ack_timeout(padapter, pmgntframe, 10);
	} else {
		rtl8188fu_dump_mgntframe(padapter, pmgntframe);
		ret = _SUCCESS;
	}
...
{
	struct sta_info *ptdls_sta = NULL, *psta = rtl8188fu_rtw_get_stainfo(&(padapter->stapriv), get_bssid(&(padapter->mlmepriv)));
	...
	psa = get_sa(ptr);
	ptdls_sta = rtl8188fu_rtw_get_stainfo(&(padapter->stapriv), psa);
	if (ptdls_sta != NULL)
	...
	if (ptdls_sta != NULL) {
		...
		if (ptdlsinfo->sta_maximum == _TRUE && ptdls_sta->alive_count >= 1 ) {
			if (rtl8188fu__rtw_memcmp(ptdlsinfo->ss_record.macaddr, empty_addr, ETH_ALEN)) {
				rtl8188fu__rtw_memcpy(ptdlsinfo->ss_record.macaddr, psa, ETH_ALEN);
		...
		if (ptdlsinfo->sta_maximum == _TRUE) {
			if (rtl8188fu__rtw_memcmp( ptdlsinfo->ss_record.macaddr, empty_addr, ETH_ALEN)) {
				...
				if (pattrib->phy_info.RxPWDBAll > ptdlsinfo->ss_record.RxPWDBAll) {
					rtl8188fu__rtw_memcpy(txmgmt.peer, ptdlsinfo->ss_record.macaddr, ETH_ALEN);
		...
		if (pattrib->phy_info.RxPWDBAll + TDLS_SIGNAL_THRESH >= UndecoratedSmoothedPWDB) {
			...
			rtl8188fu__rtw_memcpy(txmgmt.peer, psa, ETH_ALEN);
...
sint On_TDLS_Setup_Req(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	rtl8188fu__rtw_memset(&txmgmt, 0x00, sizeof(struct tdls_txmgmt));
	psa = get_sa(ptr);
	ptdls_sta = rtl8188fu_rtw_get_stainfo(pstapriv, psa);
	...
	if (ptdls_sta == NULL) {
		ptdls_sta = rtl8188fu_rtw_alloc_stainfo(pstapriv, psa);
	} else {
		if (ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE) {
	...
	if (ptdls_sta) {
		...
		for (j=FIXED_IE; j<parsing_length;) {
			...
			case _SUPPORTEDRATES_IE_:
				rtl8188fu__rtw_memcpy(supportRate, pIE->data, pIE->Length);
			...
			case _EXT_SUPPORTEDRATES_IE_:
				if (supportRateNum<=sizeof(supportRate)) {
					rtl8188fu__rtw_memcpy(supportRate+supportRateNum, pIE->data, pIE->Length);
					supportRateNum += pIE->Length;
				}
				break;
			...
			case _RSN_IE_2_:
				...
				if (prx_pkt_attrib->encrypt) {
					...
					rtl8188fu__rtw_memcpy(ptdls_sta->TDLS_RSNIE, pIE->data, pIE->Length);
					pairwise_count = *(u16*)(ppairwise_cipher-2);
					for (k=0; k<pairwise_count; k++) {
						if (rtl8188fu__rtw_memcmp( ppairwise_cipher+4*k, rtl8188fu_RSN_CIPHER_SUITE_CCMP, 4)==_TRUE)
							ccmp_included=1;
					}
			...
			case _FTIE_:
				if (prx_pkt_attrib->encrypt)
					rtl8188fu__rtw_memcpy(SNonce, (ptr+j+52), 32);
				break;
			...
			case EID_BSSCoexistence:
				break;
			case _LINK_ID_IE_:
				if (rtl8188fu__rtw_memcmp(get_bssid(pmlmepriv), pIE->data, 6) == _FALSE)
		...
		ptdls_sta->tdls_sta_state|= TDLS_INITIATOR_STATE;
		if (prx_pkt_attrib->encrypt) {
			rtl8188fu__rtw_memcpy(ptdls_sta->SNonce, SNonce, 32);
		...
		rtl8188fu__rtw_memcpy(ptdls_sta->bssrateset, supportRate, supportRateNum);
	...
	rtl8188fu__rtw_memcpy(txmgmt.peer, prx_pkt_attrib->src, ETH_ALEN);
...
int On_TDLS_Setup_Rsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	rtl8188fu__rtw_memcpy(&status_code, ptr+2, 2);
	...
	for (j = FIXED_IE; j<parsing_length;) {
		...
		case _RSN_IE_2_:
			...
			/* Check CCMP pairwise_cipher presence. */
			ppairwise_cipher=prsnie+10;
			rtl8188fu__rtw_memcpy(&pairwise_count, (u16*)(ppairwise_cipher-2), 2);
			for (k=0;k<pairwise_count;k++) {
				if (rtl8188fu__rtw_memcmp( ppairwise_cipher+4*k, rtl8188fu_RSN_CIPHER_SUITE_CCMP, 4) == _TRUE)
					verify_ccmp=1;
			}
		case _EXT_CAP_IE_:
			break;
		case _VENDOR_SPECIFIC_IE_:
			if (rtl8188fu__rtw_memcmp((u8 *)pIE + 2, rtl8188fu_WMM_INFO_OUI, 6) == _TRUE) {	
				/* WMM Info ID and OUI */
		...
		case _FTIE_:
			pftie=(u8*)pIE;
			rtl8188fu__rtw_memcpy(ANonce, (ptr+j+20), 32);
			break;
		case _TIMEOUT_ITVL_IE_:
			ptimeout_ie=(u8*)pIE;
	...
	ptdls_sta->bssratelen = supportRateNum;
	rtl8188fu__rtw_memcpy(ptdls_sta->bssrateset, supportRate, supportRateNum);
	rtl8188fu__rtw_memcpy(ptdls_sta->ANonce, ANonce, 32);
	...
	if (rtw_tdls_is_driver_setup(padapter) == _TRUE) {
		...
		if (txmgmt.status_code == _STATS_SUCCESSFUL_) {
			...
			rtl8188fu_rtw_tdls_cmd(padapter, ptdls_sta->hwaddr, TDLS_ESTABLISHED);
	...
	for (j = FIXED_IE; j < parsing_length;) {
		...
		switch (pIE->ElementID) {
			case _RSN_IE_2_:
				...
				break;
			case _VENDOR_SPECIFIC_IE_:
				if (rtl8188fu__rtw_memcmp((u8 *)pIE + 2, rtl8188fu_WMM_PARA_OUI, 6) == _TRUE) {	
...
int On_TDLS_Dis_Req(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	rtl8188fu__rtw_memcpy(&txmgmt.peer, precv_frame->u.hdr.attrib.src, ETH_ALEN);
	...
	for (j=FIXED_IE; j<parsing_length;) {
		...
		switch (pIE->ElementID) {
		case _LINK_ID_IE_:
			psta_ap = rtl8188fu_rtw_get_stainfo(pstapriv, pIE->data);
			if (psta_ap == NULL)
				goto exit;
			dst = pIE->data + 12;
			if (MacAddr_isBcst(dst) == _FALSE && (rtl8188fu__rtw_memcmp(adapter_mac_addr(padapter), dst, 6) == _FALSE))
				goto exit;
			break;
		default:
			break;
...
int On_TDLS_Teardown(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (ptdls_sta != NULL) {
		if (rtw_tdls_is_driver_setup(padapter))
			rtl8188fu_rtw_tdls_cmd(padapter, ptdls_sta->hwaddr, TDLS_TEARDOWN_STA_LOCALLY);
	}
...
{
	struct rx_pkt_attrib	*pattrib = &precv_frame->u.hdr.attrib;
	struct sta_info *ptdls_sta = rtl8188fu_rtw_get_stainfo(&padapter->stapriv, pattrib->src);	
	u8 *ptr = precv_frame->u.hdr.rx_data;
	struct tdls_txmgmt txmgmt;
	...
	if (ptdls_sta != NULL) {
		...
		//rtl8188fu_issue_nulldata_to_TDLS_peer_STA(padapter, ptdls_sta->hwaddr, 0, 0, 0);
	} else {
...
int On_TDLS_Peer_Traffic_Rsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	struct rx_pkt_attrib	*pattrib = & precv_frame->u.hdr.attrib;
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct sta_info *ptdls_sta = rtl8188fu_rtw_get_stainfo(pstapriv, pattrib->src);
	u8 wmmps_ac=0;
...
sint On_TDLS_Ch_Switch_Req(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if ((*(ptr + 2) == 2) && (rtl8188fu_hal_is_band_support(padapter, BAND_ON_5G))) {
	...
	if (rtl8188fu__rtw_memcmp(pchsw_info->addr, zaddr, ETH_ALEN) == _TRUE)
		rtl8188fu__rtw_memcpy(pchsw_info->addr, ptdls_sta->hwaddr, ETH_ALEN);

	if (rtl8188fu_ATOMIC_READ(&pchsw_info->chsw_on) == _FALSE)
		rtl8188fu_rtw_tdls_cmd(padapter, ptdls_sta->hwaddr, TDLS_CH_SW_START);
	
	rtl8188fu_rtw_tdls_cmd(padapter, ptdls_sta->hwaddr, TDLS_CH_SW_RESP);
...
sint On_TDLS_Ch_Switch_Rsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (rtl8188fu_ATOMIC_READ(&pchsw_info->chsw_on) == _TRUE) {
		if (pmlmeext->cur_channel != rtl8188fu_rtw_get_oper_ch(padapter)) {
	...
	if (status_code != 0) {
		...
		rtl8188fu_rtw_tdls_cmd(padapter, ptdls_sta->hwaddr, TDLS_CH_SW_END);
		ret = _FAIL;
		goto exit;
	}
	...
	for (j = FIXED_IE; j < parsing_length;) {
		...
		case _LINK_ID_IE_:
			break;
		case _CH_SWITCH_TIMING_:
			rtl8188fu__rtw_memcpy(&switch_time, pIE->data, 2);
			if (switch_time > ptdls_sta->ch_switch_time)
				rtl8188fu__rtw_memcpy(&ptdls_sta->ch_switch_time, &switch_time, 2);

			rtl8188fu__rtw_memcpy(&switch_timeout, pIE->data + 2, 2);
			if (switch_timeout > ptdls_sta->ch_switch_timeout)
				rtl8188fu__rtw_memcpy(&ptdls_sta->ch_switch_timeout, &switch_timeout, 2);
			break;
		default:
			break;
		}
...
void wfd_ie_tdls(_adapter * padapter, u8 *pframe, u32 *pktlen )
{
	...
	/* IPv4 Address */
	rtl8188fu__rtw_memcpy( wfdie + wfdielen, pwfd_info->ip_address, 4 );
	wfdielen += 4;
	
	pframe = rtl8188fu_rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_, wfdielen, (unsigned char *) wfdie, pktlen);
...
void rtw_build_tdls_setup_req_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt)
{
	...
	struct sta_info *ptdls_sta=rtl8188fu_rtw_get_stainfo( (&padapter->stapriv) , pattrib->dst);
	...
	if (pattrib->encrypt) {
		for (i=0;i<8;i++) {
			time=rtl8188fu_rtw_get_current_time();
			rtl8188fu__rtw_memcpy(&ptdls_sta->SNonce[4*i], (u8 *)&time, 4);
		}
	}
...
void rtw_build_tdls_setup_rsp_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt)
{
	...
	if (pattrib->encrypt && ptdls_sta != NULL) {
		for (k=0;k<8;k++) {
			time = rtl8188fu_rtw_get_current_time();
			rtl8188fu__rtw_memcpy(&ptdls_sta->ANonce[4*k], (u8*)&time, 4);
		}
	}
...
void rtw_build_tdls_peer_traffic_indication_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt)
{
	...
	if (ptdls_sta->uapsd_vo & BIT(1))
		AC_queue=BIT(3);
	pframe = rtl8188fu_rtw_set_ie(pframe, _PTI_BUFFER_STATUS_, 1, &AC_queue, &(pattrib->pktlen));
...
void rtw_build_tdls_ch_switch_req_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt)
{
	...
	struct pkt_attrib	*pattrib = &pxmitframe->attrib;
	struct sta_priv 	*pstapriv = &padapter->stapriv;
	struct sta_info *ptdls_sta = rtl8188fu_rtw_get_stainfo(pstapriv, pattrib->dst);
	u16 switch_time= TDLS_CH_SWITCH_TIME * 1000, switch_timeout=TDLS_CH_SWITCH_TIMEOUT * 1000;
...
void rtw_build_tunneled_probe_req_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe)
{
	...
	pframe = rtl8188fu_rtw_set_fixed_ie(pframe, 3, WFA_OUI, &(pattrib->pktlen));
	pframe = rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(probe_req), &(pattrib->pktlen));

	if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
		wfdielen = rtl8188fu_build_probe_req_wfd_ie(pwdinfo, pframe);
		pframe += wfdielen;
		pattrib->pktlen += wfdielen;
	} else if (!rtw_p2p_chk_state(pbuddy_wdinfo, P2P_STATE_NONE)) {
		wfdielen = rtl8188fu_build_probe_req_wfd_ie(pbuddy_wdinfo, pframe);
		pframe += wfdielen;
		pattrib->pktlen += wfdielen;
	}
...
void rtw_build_tunneled_probe_rsp_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe)
{
	...
	pframe = rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(category), &(pattrib->pktlen));
	pframe = rtl8188fu_rtw_set_fixed_ie(pframe, 3, WFA_OUI, &(pattrib->pktlen));
	pframe = rtl8188fu_rtw_set_fixed_ie(pframe, 1, &(probe_rsp), &(pattrib->pktlen));

	if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
		wfdielen = rtl8188fu_build_probe_resp_wfd_ie(pwdinfo, pframe, 1);
		pframe += wfdielen;
		pattrib->pktlen += wfdielen;
	} else if (!rtw_p2p_chk_state(pbuddy_wdinfo, P2P_STATE_NONE)) {
		wfdielen = rtl8188fu_build_probe_resp_wfd_ie(pbuddy_wdinfo, pframe, 1);
		pframe += wfdielen;
		pattrib->pktlen += wfdielen;
	}
...
void _tdls_ch_switch_timer_hdl(void *FunctionContext)
{
	...
	DBG_871X("[TDLS] %s, can't get traffic from op_ch:%d\n", __FUNCTION__, rtl8188fu_rtw_get_oper_ch(padapter));
}
...
void _tdls_delay_timer_hdl(void *FunctionContext)
{
	...
	DBG_871X("[TDLS] %s, op_ch:%d, tdls_state:0x%08x\n", __FUNCTION__, rtl8188fu_rtw_get_oper_ch(padapter), ptdls_sta->tdls_sta_state);
...
void _tdls_handshake_timer_hdl(void *FunctionContext)
{
	...
	if (ptdls_sta != NULL) {
		...
		if (ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE) 
		{
			rtl8188fu_rtw_tdls_cmd(padapter, ptdls_sta->hwaddr, TDLS_TEARDOWN_STA);
		}
		else
		{
			rtl8188fu_rtw_tdls_cmd(padapter, ptdls_sta->hwaddr, TDLS_TEARDOWN_STA_LOCALLY);
...
{
	return rtl8188fu_query_ra_short_GI(psta);
}
