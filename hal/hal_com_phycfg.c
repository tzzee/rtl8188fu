...
/*
* rtl8188fu_rtw_regsty_get_target_tx_power -
*
* Return dBm or -1 for undefined
*/
s8 rtl8188fu_rtw_regsty_get_target_tx_power(
	IN	PADAPTER		Adapter,
	IN	u8				Band,
	IN	u8				RfPath,
...
bool rtl8188fu_rtw_regsty_chk_target_tx_power_valid(_adapter *adapter)
{
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		if (!rtl8188fu_hal_is_band_support(adapter, band))
			continue;
		...
		for (path = 0; path < RF_PATH_MAX; path++) {
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				target = rtl8188fu_rtw_regsty_get_target_tx_power(adapter, band, path, rs);
				if (target == -1)
					return _FALSE;
			}
		}
...
VOID
phy_rtl8188fu_SetTxPowerByRateBase(
	IN	PADAPTER		Adapter,
	IN	u8				Band,
	IN	u8				RfPath,
	IN	RATE_SECTION	RateSection,
	...
	)
{
	...
	if (rtl8188fu_phy_is_tx_power_by_rate_needed(Adapter) == _FALSE && regsty->target_tx_pwr_valid == _TRUE)
		target_power = 2 * rtl8188fu_rtw_regsty_get_target_tx_power(Adapter, Band, RfPath, RateSection);
	else
		target_power = rtl8188fu_PHY_GetTxPowerByRateBase(Adapter, Band, RfPath, rate_section_to_tx_num(RateSection), RateSection);
...
VOID
rtl8188fu_phy_StoreTxPowerByRateBaseOld(	
	...
	)
{
	...
	phy_rtl8188fu_SetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_A, CCK, RF_1TX, base );

	rawValue = ( u16 ) ( pHalData->MCSTxPowerLevelOriginalOffset[0][1] >> 24 ) & 0xFF; 
	base = ( rawValue >> 4 ) * 10 + ( rawValue & 0xF );
	phy_rtl8188fu_SetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_A, OFDM, RF_1TX, base );

	rawValue = ( u16 ) ( pHalData->MCSTxPowerLevelOriginalOffset[0][3] >> 24 ) & 0xFF; 
	base = ( rawValue >> 4 ) * 10 + ( rawValue & 0xF );
	phy_rtl8188fu_SetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_A, HT_MCS0_MCS7, RF_1TX, base );

	rawValue = ( u16 ) ( pHalData->MCSTxPowerLevelOriginalOffset[0][5] >> 24 ) & 0xFF; 
	base = ( rawValue >> 4) * 10 + ( rawValue & 0xF );
	phy_rtl8188fu_SetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_A, HT_MCS8_MCS15, RF_2TX, base );

	rawValue = ( u16 ) ( pHalData->MCSTxPowerLevelOriginalOffset[0][7] & 0xFF ); 
	base = ( rawValue >> 4 ) * 10 + ( rawValue & 0xF );
	phy_rtl8188fu_SetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_B, CCK, RF_1TX, base );

	rawValue = ( u16 ) ( pHalData->MCSTxPowerLevelOriginalOffset[0][9] >> 24 ) & 0xFF; 
	base = ( rawValue >> 4 ) * 10 + ( rawValue & 0xF );
	phy_rtl8188fu_SetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_B, OFDM, RF_1TX, base );

	rawValue = ( u16 ) ( pHalData->MCSTxPowerLevelOriginalOffset[0][11] >> 24 ) & 0xFF; 
	base = ( rawValue >> 4 ) * 10 + ( rawValue & 0xF );
	phy_rtl8188fu_SetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_B, HT_MCS0_MCS7, RF_1TX, base );

	rawValue = ( u16 ) ( pHalData->MCSTxPowerLevelOriginalOffset[0][13] >> 24 ) & 0xFF; 
	base = ( rawValue >> 4 ) * 10 + ( rawValue & 0xF );
	phy_rtl8188fu_SetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_B, HT_MCS8_MCS15, RF_2TX, base );
}
#endif /* TX_POWER_BY_RATE_OLD */

VOID
rtl8188fu_phy_StoreTxPowerByRateBase(	
	IN	PADAPTER	pAdapter
	)
{
		...
		for (path = RF_PATH_A; path < RF_PATH_MAX; path++) {
			...
			*/
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				base = rtl8188fu__rtl8188fu_PHY_GetTxPowerByRate(pAdapter, band, path, tx_num, rate_sec_base[rs]);
				phy_rtl8188fu_SetTxPowerByRateBase(pAdapter, band, path, rs, tx_num, base);
			}
		}
	}
}
...
void
rtl8188fu_PHY_StoreTxPowerByRateNew(
	IN	PADAPTER	pAdapter,
	IN	u32			Band,
	IN	u32			RfPath,
	IN	u32			TxNum,
	...
	)
{
	...
	rtl8188fu_PHY_GetRateValuesOfTxPowerByRate(pAdapter, RegAddr, BitMask, Data, rates, PwrByRateVal, &rateNum);
	...
	for (i = 0; i < rateNum; ++i) {
		u8 rate_idx = rtl8188fu_PHY_GetRateIndexOfTxPowerByRate(rates[i]);
...
#ifdef TX_POWER_BY_RATE_OLD
void 
rtl8188fu_PHY_StoreTxPowerByRateOld(
	IN	PADAPTER		pAdapter,
	IN	u32				RegAddr,
	IN	u32				BitMask,
	IN	u32				Data
	...
	)
{
	...
	if ( pDM_Odm->PhyRegPgVersion > 0 )
	{
		rtl8188fu_PHY_StoreTxPowerByRateNew( pAdapter, Band, RfPath, TxNum, RegAddr, BitMask, Data );
	}
#ifdef TX_POWER_BY_RATE_OLD
	else if ( pDM_Odm->PhyRegPgVersion == 0 )
	{
		rtl8188fu_PHY_StoreTxPowerByRateOld( pAdapter, RegAddr, BitMask, Data );
	...
	)
{
	...
	// CCK
	base = rtl8188fu_PHY_GetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_A, RF_1TX, CCK );
	phy_ConvertTxPowerByRateByBase( 
	...
	// OFDM
	base = rtl8188fu_PHY_GetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_A, RF_1TX, OFDM );
	phy_ConvertTxPowerByRateByBase( 
	...
	// HT MCS0~7
	base = rtl8188fu_PHY_GetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_A, RF_1TX, HT_MCS0_MCS7 );
	phy_ConvertTxPowerByRateByBase( 
	...
	// HT MCS8~15
	base = rtl8188fu_PHY_GetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_A, RF_2TX, HT_MCS8_MCS15 );
	phy_ConvertTxPowerByRateByBase( 
	...
	// CCK
	base = rtl8188fu_PHY_GetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_B, RF_1TX, CCK );
	phy_ConvertTxPowerByRateByBase( 
	...
	// OFDM
	base = rtl8188fu_PHY_GetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_B, RF_1TX, OFDM );
	phy_ConvertTxPowerByRateByBase( 
	...
	// HT MCS0~7
	base = rtl8188fu_PHY_GetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_B, RF_1TX, HT_MCS0_MCS7 );
	phy_ConvertTxPowerByRateByBase( 
	...
	// HT MCS8~15
	base = rtl8188fu_PHY_GetTxPowerByRateBase( pAdapter, BAND_ON_2_4G, ODM_RF_PATH_B, RF_2TX, HT_MCS8_MCS15 );
	phy_ConvertTxPowerByRateByBase( 
...
VOID
rtl8188fu_phy_ConvertTxPowerByRateInDbmToRelativeValues(
	IN	PADAPTER	pAdapter
	)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA( pAdapter );
	...
	for ( band = BAND_ON_2_4G; band <= BAND_ON_5G; ++band )
	{
		for ( path = ODM_RF_PATH_A; path <= ODM_RF_PATH_D; ++path )
		{
			...
			{
				// CCK
				base = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, MGN_11M );
				for ( i = 0; i < sizeof( cckRates ); ++i )
				{
					value = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, cckRates[i] );
					PHY_rtl8188fu_SetTxPowerByRate( pAdapter, band, path, txNum, cckRates[i], value - base );
				}

				// OFDM
				base = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, MGN_54M );
				for ( i = 0; i < sizeof( ofdmRates ); ++i )
				{
					value = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, ofdmRates[i] );
					PHY_rtl8188fu_SetTxPowerByRate( pAdapter, band, path, txNum, ofdmRates[i], value - base );
				}
				
				// HT MCS0~7
				base = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, MGN_MCS7 );
				for ( i = 0; i < sizeof( mcs0_7Rates ); ++i )
				{
					value = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, mcs0_7Rates[i] );
					PHY_rtl8188fu_SetTxPowerByRate( pAdapter, band, path, txNum, mcs0_7Rates[i], value - base );
				}

				// HT MCS8~15
				base = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, MGN_MCS15 );
				for ( i = 0; i < sizeof( mcs8_15Rates ); ++i )
				{
					value = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, mcs8_15Rates[i] );
					PHY_rtl8188fu_SetTxPowerByRate( pAdapter, band, path, txNum, mcs8_15Rates[i], value - base );
				}

				// HT MCS16~23
				base = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, MGN_MCS23 );
				for ( i = 0; i < sizeof( mcs16_23Rates ); ++i )
				{
					value = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, mcs16_23Rates[i] );
					PHY_rtl8188fu_SetTxPowerByRate( pAdapter, band, path, txNum, mcs16_23Rates[i], value - base );
				}

				// VHT 1SS
				base = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, MGN_VHT1SS_MCS7 );
				for ( i = 0; i < sizeof( vht1ssRates ); ++i )
				{
					value = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, vht1ssRates[i] );
					PHY_rtl8188fu_SetTxPowerByRate( pAdapter, band, path, txNum, vht1ssRates[i], value - base );
				}

				// VHT 2SS
				base = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, MGN_VHT2SS_MCS7 );
				for ( i = 0; i < sizeof( vht2ssRates ); ++i )
				{
					value = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, vht2ssRates[i] );
					PHY_rtl8188fu_SetTxPowerByRate( pAdapter, band, path, txNum, vht2ssRates[i], value - base );
				}

				// VHT 3SS
				base = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, MGN_VHT3SS_MCS7 );
				for ( i = 0; i < sizeof( vht3ssRates ); ++i )
				{
					value = rtl8188fu_PHY_GetTxPowerByRate( pAdapter, band, path, txNum, vht3ssRates[i] );
					PHY_rtl8188fu_SetTxPowerByRate( pAdapter, band, path, txNum, vht3ssRates[i], value - base );
				}
			}
		}
	}
	...
	)
{
	...
	rtl8188fu_phy_StoreTxPowerByRateBase( pAdapter );
	rtl8188fu_phy_ConvertTxPowerByRateInDbmToRelativeValues( pAdapter );
}
	...
	)
{
	...
	if ( RateSection == CCK )
	{
		...
		if ( pHalData->CurrentBandType == BAND_ON_2_4G )
			PHY_rtl8188fu_SetTxPowerIndexByRateArray( pAdapter, RFPath, pHalData->CurrentChannelBW, Channel,
	...
	if (Channel <= 14) {
		...
		*ChannelIdx = Channel - 1;
		...
		for (i = 0; i < CENTER_CH_5G_ALL_NUM; ++i) {
			if (rtl8188fu_center_ch_5g_all[i] == Channel) {
				*ChannelIdx = i;
				return bIn24G;
			}
		}
	...
	)
{
	...
	if (rtl8188fu_HAL_IsLegalChannel(pAdapter, Channel) == _FALSE)
	{
		chnlIdx = 0;
		DBG_871X("Illegal channel!!\n");
	}

	*bIn24G = rtl8188fu_phy_GetChnlIndex(Channel, &chnlIdx);
	...
	if (*bIn24G) //3 ============================== 2.4 G ==============================
	{
		...
		{
			DBG_871X("rtl8188fu_PHY_GetTxPowerIndexBase: INVALID Rate.\n");
		}
	...
	else //3 ============================== 5 G ==============================
	{
		...
		{
			DBG_871X("===> rtl8188fu_mpt_ProQueryCalTxPower_Jaguar: INVALID Rate.\n");
		}
		...
		else if (BandWidth== CHANNEL_WIDTH_80)
		{
			...
			for (i = 0; i < CENTER_CH_5G_80M_NUM; ++i)
				if (rtl8188fu_center_ch_5g_80m[i] == Channel)
					chnlIdx = i;
	...
	)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(pAdapter);
	s8 value = 0;
	u8 rateIndex = rtl8188fu_PHY_GetRateIndexOfTxPowerByRate(Rate);
...
{
	if (!rtl8188fu_phy_is_tx_power_by_rate_needed(pAdapter))
		return 0;

	return rtl8188fu__rtl8188fu_PHY_GetTxPowerByRate(pAdapter, Band, RFPath, TxNum, Rate);
}
	...
	)
{
	...
	{
		if ( bIsIn24G )
			PHY_rtl8188fu_SetTxPowerIndexByRateSection( Adapter, path, channel, CCK );
		
		PHY_rtl8188fu_SetTxPowerIndexByRateSection( Adapter, path, channel, OFDM );
		PHY_rtl8188fu_SetTxPowerIndexByRateSection( Adapter, path, channel, HT_MCS0_MCS7 );

		if (IS_HARDWARE_TYPE_JAGUAR(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter))
			PHY_rtl8188fu_SetTxPowerIndexByRateSection(Adapter, path, channel, VHT_1SSMCS0_1SSMCS9);

		if (pHalData->NumTotalRFPath >= 2)
		{
			PHY_rtl8188fu_SetTxPowerIndexByRateSection( Adapter, path, channel, HT_MCS8_MCS15 );

			if (IS_HARDWARE_TYPE_JAGUAR(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter))
				PHY_rtl8188fu_SetTxPowerIndexByRateSection(Adapter, path, channel, VHT_2SSMCS0_2SSMCS9);

			if (IS_HARDWARE_TYPE_8814A(Adapter))
			{
				PHY_rtl8188fu_SetTxPowerIndexByRateSection( Adapter, path, channel, HT_MCS16_MCS23 );
				PHY_rtl8188fu_SetTxPowerIndexByRateSection( Adapter, path, channel, VHT_3SSMCS0_3SSMCS9 );
			}
		}
	}
}
	...
	)
{
	...
	{
		powerIndex = rtl8188fu_PHY_GetTxPowerIndex(pAdapter, RFPath, Rates[i], BandWidth, Channel);
		PHY_rtl8188fu_SetTxPowerIndex(pAdapter, powerIndex, RFPath, Rates[i]);
	}
}

s8
rtl8188fu_phy_GetWorldWideLimit(
	s8* LimitTable
)
{
	s8	min = LimitTable[0];
...
s8
rtl8188fu_phy_GetChannelIndexOfTxPowerLimit(
	IN	u8			Band,
	IN	u8			Channel
	)
{
	...
	)
{
	...
	if ( Band == BAND_ON_2_4G )
		channel = rtl8188fu_phy_GetChannelIndexOfTxPowerLimit( BAND_ON_2_4G, Channel );
	else if ( Band == BAND_ON_5G )
		channel = rtl8188fu_phy_GetChannelIndexOfTxPowerLimit( BAND_ON_5G, Channel );
	...
	if ( Band == BAND_ON_2_4G ) {
		...
		powerLimit = (regulation == TXPWR_LMT_WW) ? rtl8188fu_phy_GetWorldWideLimit(limits) :
...
VOID
rtl8188fu_phy_CrossReferenceHTAndVHTTxPowerLimit(
	IN	PADAPTER			pAdapter
	)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(pAdapter);
					...
					if (bw == CHANNEL_WIDTH_20 || bw == CHANNEL_WIDTH_40) {
						...
						if (0)
							DBG_871X("reg:%u, bw:%u, ch:%u, %s ref %s\n"
								...
								, rtl8188fu_rate_section_str(rs), rtl8188fu_rate_section_str(ref_rs));
	...
	)
{
	...
	rtl8188fu_phy_CrossReferenceHTAndVHTTxPowerLimit(Adapter);
...
/*
* rtl8188fu_PHY_InitTxPowerLimit - Set all hal_data.TxPwrLimit_2_4G, TxPwrLimit_5G array to MAX_POWER_INDEX
*/
VOID
...
/*
* rtl8188fu_PHY_rtl8188fu_SetTxPowerLimit - Parsing TX power limit from phydm array, called by odm_ConfigBB_TXPWR_LMT_XXX in phydm
*/
VOID
	...
	)
{
	...
	if (rtl8188fu_GetU1ByteIntegerFromStringInDecimal((s8 *)Channel, &channel) == _FALSE
		|| rtl8188fu_GetU1ByteIntegerFromStringInDecimal((s8 *)PowerLimit, &powerLimit) == _FALSE
	){
	...
	if (rtl8188fu_eqNByte(Regulation, (u8 *)("FCC"), 3))
		regulation = TXPWR_LMT_FCC;
	else if (rtl8188fu_eqNByte(Regulation, (u8 *)("MKK"), 3))
		regulation = TXPWR_LMT_MKK;
	else if (rtl8188fu_eqNByte(Regulation, (u8 *)("ETSI"), 4))
		regulation = TXPWR_LMT_ETSI;
	else if (rtl8188fu_eqNByte(Regulation, (u8 *)("WW13"), 4))
		regulation = TXPWR_LMT_WW;
	else {
	...
	if (rtl8188fu_eqNByte(RateSection, (u8 *)("CCK"), 3) && rtl8188fu_eqNByte(RfPath, (u8 *)("1T"), 2))
		rateSection = CCK;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("OFDM"), 4) && rtl8188fu_eqNByte(RfPath, (u8 *)("1T"), 2))
		rateSection = OFDM;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("HT"), 2) && rtl8188fu_eqNByte(RfPath, (u8 *)("1T"), 2))
		rateSection = HT_1SS;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("HT"), 2) && rtl8188fu_eqNByte(RfPath, (u8 *)("2T"), 2))
		rateSection = HT_2SS;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("HT"), 2) && rtl8188fu_eqNByte(RfPath, (u8 *)("3T"), 2))
		rateSection = HT_3SS;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("HT"), 2) && rtl8188fu_eqNByte(RfPath, (u8 *)("4T"), 2))
		rateSection = HT_4SS;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("VHT"), 3) && rtl8188fu_eqNByte(RfPath, (u8 *)("1T"), 2))
		rateSection = VHT_1SS;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("VHT"), 3) && rtl8188fu_eqNByte(RfPath, (u8 *)("2T"), 2))
		rateSection = VHT_2SS;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("VHT"), 3) && rtl8188fu_eqNByte(RfPath, (u8 *)("3T"), 2))
		rateSection = VHT_3SS;
	else if (rtl8188fu_eqNByte(RateSection, (u8 *)("VHT"), 3) && rtl8188fu_eqNByte(RfPath, (u8 *)("4T"), 2))
		rateSection = VHT_4SS;
	else {
	...
	if (rtl8188fu_eqNByte(Bandwidth, (u8 *)("20M"), 3))
		bandwidth = CHANNEL_WIDTH_20;
	else if (rtl8188fu_eqNByte(Bandwidth, (u8 *)("40M"), 3))
		bandwidth = CHANNEL_WIDTH_40;
	else if (rtl8188fu_eqNByte(Bandwidth, (u8 *)("80M"), 3))
	...
	if (rtl8188fu_eqNByte(Band, (u8 *)("2.4G"), 4)) {
		channelIndex = rtl8188fu_phy_GetChannelIndexOfTxPowerLimit(BAND_ON_2_4G, channel);
	...
	} else if (rtl8188fu_eqNByte(Band, (u8 *)("5G"), 2)) {

		channelIndex = rtl8188fu_phy_GetChannelIndexOfTxPowerLimit(BAND_ON_5G, channel);
	...
	)
{
	...
	if (IS_HARDWARE_TYPE_8814A(pAdapter)) {
		...
		txPower = rtl8188fu_PHY_GetTxPowerIndex_8814A(pAdapter, RFPath, Rate, BandWidth, Channel);
#endif
	} else if (IS_HARDWARE_TYPE_JAGUAR(pAdapter)) {
#if ((RTL8812A_SUPPORT == 1) || (RTL8821A_SUPPORT == 1))
		txPower = rtl8188fu_PHY_GetTxPowerIndex_8812A(pAdapter, RFPath, Rate, BandWidth, Channel);
#endif
	}
	else if (IS_HARDWARE_TYPE_8723B(pAdapter)) {
#if (RTL8723B_SUPPORT == 1)
		txPower = rtl8188fu_PHY_GetTxPowerIndex_8723B(pAdapter, RFPath, Rate, BandWidth, Channel);
#endif
	}
	else if (IS_HARDWARE_TYPE_8703B(pAdapter)) {
#if (RTL8703B_SUPPORT == 1)
		txPower = rtl8188fu_PHY_GetTxPowerIndex_8703B(pAdapter, RFPath, Rate, BandWidth, Channel);
#endif
	}
	else if (IS_HARDWARE_TYPE_8192E(pAdapter)) {
#if (RTL8192E_SUPPORT==1)
		txPower = rtl8188fu_PHY_GetTxPowerIndex_8192E(pAdapter, RFPath, Rate, BandWidth, Channel);
#endif
	}
	else if (IS_HARDWARE_TYPE_8188E(pAdapter)) {
#if (RTL8188E_SUPPORT==1)
		txPower = rtl8188fu_PHY_GetTxPowerIndex_8188E(pAdapter, RFPath, Rate, BandWidth, Channel);
#endif
	} else if (IS_HARDWARE_TYPE_8188F(pAdapter)) {
#if (RTL8188F_SUPPORT == 1)
		txPower = rtl8188fu_PHY_GetTxPowerIndex_8188F(pAdapter, RFPath, Rate, BandWidth, Channel);
#endif
	}
	...
	)
{
	if (IS_HARDWARE_TYPE_8814A(pAdapter)) {
#if (RTL8814A_SUPPORT == 1)
		PHY_rtl8188fu_SetTxPowerIndex_8814A(pAdapter, PowerIndex, RFPath, Rate);
#endif
	}
	else if (IS_HARDWARE_TYPE_JAGUAR(pAdapter)) {
#if ((RTL8812A_SUPPORT==1) || (RTL8821A_SUPPORT == 1))
		PHY_rtl8188fu_SetTxPowerIndex_8812A( pAdapter, PowerIndex, RFPath, Rate );
#endif
	}
	else if (IS_HARDWARE_TYPE_8723B(pAdapter)) {
#if (RTL8723B_SUPPORT==1)
		PHY_rtl8188fu_SetTxPowerIndex_8723B( pAdapter, PowerIndex, RFPath, Rate );
#endif
	}
	else if (IS_HARDWARE_TYPE_8703B(pAdapter)) {
#if (RTL8703B_SUPPORT==1)
		PHY_rtl8188fu_SetTxPowerIndex_8703B( pAdapter, PowerIndex, RFPath, Rate );
#endif
	}
	else if (IS_HARDWARE_TYPE_8192E(pAdapter)) {
#if (RTL8192E_SUPPORT==1)
		PHY_rtl8188fu_SetTxPowerIndex_8192E( pAdapter, PowerIndex, RFPath, Rate );
#endif
	}
	else if (IS_HARDWARE_TYPE_8188E(pAdapter)) {
#if (RTL8188E_SUPPORT==1)
		PHY_rtl8188fu_SetTxPowerIndex_8188E( pAdapter, PowerIndex, RFPath, Rate );
#endif
	} else if (IS_HARDWARE_TYPE_8188F(pAdapter)) {
#if (RTL8188F_SUPPORT == 1)
		PHY_rtl8188fu_SetTxPowerIndex_8188F(pAdapter, PowerIndex, RFPath, Rate);
#endif
	}
}
...
int rtl8188fu_phy_load_tx_power_by_rate(_adapter *adapter, u8 chk_file)
{
	...
	rtl8188fu_PHY_InitTxPowerByRate(adapter);

	/* tx power limit is based on tx power by rate */
	hal_data->txpwr_limit_loaded = 0;

#ifdef CONFIG_EMBEDDED_FWIMG
	if (HAL_STATUS_SUCCESS == rtl8188fu_ODM_ConfigBBWithHeaderFile(&hal_data->odmpriv, CONFIG_BB_PHY_REG_PG)) {
...
post_hdl:
	...
	rtl8188fu_PHY_TxPowerByRateConfiguration(adapter);
...
int rtl8188fu_phy_load_tx_power_limit(_adapter *adapter, u8 chk_file)
{
	...
	hal_data->txpwr_limit_loaded = 0;
	rtl8188fu_PHY_InitTxPowerLimit(adapter);
...
#ifdef CONFIG_EMBEDDED_FWIMG
	if (HAL_STATUS_SUCCESS == rtl8188fu_ODM_ConfigRFWithHeaderFile(&hal_data->odmpriv, CONFIG_RF_TXPWR_LMT, (ODM_RF_RADIO_PATH_E)0)) {
...
post_hdl:
	rtl8188fu_PHY_ConvertTxPowerLimitToPowerIndex(adapter);
...
void rtl8188fu_phy_load_tx_power_ext_info(_adapter *adapter, u8 chk_file)
{
	...
	regsty->target_tx_pwr_valid = rtl8188fu_rtw_regsty_chk_target_tx_power_valid(adapter);

	/* power by rate and limit */
	if (rtl8188fu_phy_is_tx_power_by_rate_needed(adapter)
		|| (rtl8188fu_phy_is_tx_power_limit_needed(adapter) && regsty->target_tx_pwr_valid != _TRUE))
		rtl8188fu_phy_load_tx_power_by_rate(adapter, chk_file);

	if (rtl8188fu_phy_is_tx_power_limit_needed(adapter))
		rtl8188fu_phy_load_tx_power_limit(adapter, chk_file);
}

inline void rtl8188fu_phy_reload_tx_power_ext_info(_adapter *adapter)
{
	rtl8188fu_phy_load_tx_power_ext_info(adapter, 1);
}

inline void rtl8188fu_phy_reload_default_tx_power_ext_info(_adapter *adapter)
{
	rtl8188fu_phy_load_tx_power_ext_info(adapter, 0);
}
...
void rtl8188fu_dump_tx_power_ext_info(void *sel, _adapter *adapter)
{
	...
	DBG_871X_SEL_NL(sel, "tx_power_by_rate: %s, %s, %s\n"
		, rtl8188fu_phy_is_tx_power_by_rate_needed(adapter) ? "enabled" : "disabled"
	...
	DBG_871X_SEL_NL(sel, "tx_power_limit: %s, %s, %s\n"
		, rtl8188fu_phy_is_tx_power_limit_needed(adapter) ? "enabled" : "disabled"
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		...
		for (path = 0; path < RF_PATH_MAX; path++) {
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				if (target % 2)
					DBG_871X_SEL(sel, "%7s: %2d.5\n", rtl8188fu_rate_section_str(rs), target / 2);
				else
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		...
		for (path = 0; path < RF_PATH_MAX; path++) {
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				base = rtl8188fu_PHY_GetTxPowerByRateBase(adapter, band, path, tx_num, rs);

				DBG_871X_SEL_NL(sel, "%7s: ", rtl8188fu_rate_section_str(rs));

				/* dump power by rate in db */
				for (n = rate_num - 1; n >= 0; n--) {
					by_rate_offset = rtl8188fu_PHY_GetTxPowerByRate(adapter, band, path, tx_num, rtl8188fu_rates_by_sections[rs].rates[n]);
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		...
		for (bw = 0; bw < MAX_5G_BANDWIDTH_NUM; bw++) {
			...
			if (band == BAND_ON_2_4G)
				...
				ch_num = rtl8188fu_center_chs_5g_num(bw);
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				for (n = 0; n < ch_num; n++) {
					...
					if (band == BAND_ON_2_4G)
						...
						ch = rtl8188fu_center_chs_5g(bw, n);
					...
					limit_offset[0] = rtl8188fu_PHY_GetTxPowerLimit(adapter, 3, band, bw, RF_PATH_A, rtl8188fu_rates_by_sections[rs].rates[0], ch); /* FCC */
					limit_offset[1] = rtl8188fu_PHY_GetTxPowerLimit(adapter, 1, band, bw, RF_PATH_A, rtl8188fu_rates_by_sections[rs].rates[0], ch); /* ETSI */
					limit_offset[2] = rtl8188fu_PHY_GetTxPowerLimit(adapter, 2, band, bw, RF_PATH_A, rtl8188fu_rates_by_sections[rs].rates[0], ch); /* MKK */
					limit_offset[3] = rtl8188fu_PHY_GetTxPowerLimit(adapter, 4, band, bw, RF_PATH_A, rtl8188fu_rates_by_sections[rs].rates[0], ch); /* WW */
					...
					for (path = 0; path < RF_PATH_MAX; path++) {
						if (path >= hal_data->NumTotalRFPath)
							break;
						limit_offset[0] = rtl8188fu_PHY_GetTxPowerLimit(adapter, 3, band, bw, path, rtl8188fu_rates_by_sections[rs].rates[0], ch); /* FCC */
						limit_offset[1] = rtl8188fu_PHY_GetTxPowerLimit(adapter, 1, band, bw, path, rtl8188fu_rates_by_sections[rs].rates[0], ch); /* ETSI */
						limit_offset[2] = rtl8188fu_PHY_GetTxPowerLimit(adapter, 2, band, bw, path, rtl8188fu_rates_by_sections[rs].rates[0], ch); /* MKK */
						limit_offset[3] = rtl8188fu_PHY_GetTxPowerLimit(adapter, 4, band, bw, path, rtl8188fu_rates_by_sections[rs].rates[0], ch); /* WW */
